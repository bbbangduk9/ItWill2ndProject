...
@WebServlet("/office/getHealthAll")
//1. extends HttpServlet !!
public class GetHealthAllController extends HttpServlet {
	private static final long serialVersionUID = 1L;
	
	//3. doGet 메소드를 만든다. 
	@Override
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("::: GetHealthAll doGet() 실행됨 ");
		//한글깨짐 방지를 위한 문자타입(UTF-8) 처리
		response.setContentType("text/html;charset=UTF-8");
		
		//DB 데이터 가져오기 (DAO에 있는 메소드를 VO에 저장한다)
		List<PostVO> list = PostDAO.getPostHealth();
