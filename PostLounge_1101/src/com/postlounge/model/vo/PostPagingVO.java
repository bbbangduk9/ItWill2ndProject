...
//게시판의 페이징 처리를 위한 값을 저장 관리 (이하영 추가?)
public class PostPagingVO {
	private int nowPage = 1; //현재 페이지
	private int nowBlock = 1; //현재 블록(페이지를 담는 단위)
	
	private int numPerPage = 10; //하나의 페이지에 표시할 게시글 수
	private int pagePerBlock = 3; //블록당 표시하는 페이지 수
	
	private int totalRecord = 0; //총 게시물 수 (원본 게시글 수)
	private int totalPage = 0; //전체 페이지 수
	private int totalBlock = 0; //전체 블록 수
	
	private int begin = 0; //현재 페이지상의 시작 번호 // 글번호
	private int end = 0; //현재 페이지상의 마지막 번호 // 글번호
	
	private int beginPage = 0; //현재 블록의 시작 페이지 번호
	private int endPage = 0; //현재 블록의 끝 페이지 번호
	
	//전체 페이지 수 구하기
	//totalRecord 값을 페이지당 표시할 글의 수 값을 나누고,
	...
	public String toString() {
		return "PostPagingVO [nowPage=" + nowPage + ", nowBlock=" + nowBlock + ", numPerPage=" + numPerPage
