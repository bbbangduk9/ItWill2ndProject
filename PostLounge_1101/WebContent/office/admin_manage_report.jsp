...
<%//@@@ B. 게시글 페이징 처리를 하기 위함 @@@ //
	...
	//1. 전체 게시물 수량 구하기
	postPage.setTotalRecord(PostDAO.getTotalReportCnt());
...
<%	//현재 페이지 기준으로 DB 데이터(게시글) 가져오기
	...
	List<PostVO> list = PostDAO.manageReport(postPage.getBegin(), postPage.getEnd());
...
<link href="../css/admin_report.css?after" rel="stylesheet" type="text/css">
					...
					<c:choose>
						...
						<c:forEach var="vo" items="${list }">
							<tr>
								<td>${vo.postIdx }</td>
								<td class="align-left">
									<a href="admin_view.jsp?postIdx=${vo.postIdx }&cPage=${postPage.nowPage}">${vo.title }</a>
								</td>
								<td>${vo.postDate }</td>
							</tr>
						</c:forEach>
		...
		<tfoot>
			<tr>
				<td colspan="4">
					...
					<c:if test="${postPage.beginPage != 1 }">
						<li>
							<a href="admin_manage_report.jsp?cPage=${postPage.beginPage - 1 }">이전으로</a>
						</li>
					</c:if>
					...
					<c:if test="${pageNo != postPage.nowPage }">	
						<li>
							<a href="admin_manage_report.jsp?cPage=${pageNo }">${pageNo }</a>
						</li>
					</c:if>
					...
					<c:if test="${postPage.beginPage < postPage.totalPage }">
						<li>
							<a href="admin_manage_report.jsp?cPage=${postPage.endPage + 1 }">다음으로</a>
						</li>
					</c:if>
					</ol>
				</td>
