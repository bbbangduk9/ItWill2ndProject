...
<%
    ...
    System.out.println(">upd_ok.jsp memIdx 저장된 값 확인 : " + Arrays.toString(memIdxArray));
    ...
    if (memIdxArray != null && memIdxArray.length > 0) {
        ...
        System.out.println("::: 회원 번호 일치!! 변경작업 진행합니다");
        // DB 데이터 삭제 처리
...
<%!
    private int update(List<Integer> memIdxList) {
        ...
        try {
            result = ss.update("member.upMemIdxList", memIdxList);
            ss.commit(); // 명시적 커밋 처리
...
<script>
	...
	if (result == -999) { //암호불일치
		...
		location.href = "admin_manage_opr.jsp";
	}
	if (result == -1) { //예외발생시
		alert("[예외발생] 변경실패. 개발팀에 문의해주세요.\n" +
				"pstlng_dev@postlounge.com");
		location.href = "admin_manage_opr.jsp";
	}
	if (result == 0) { //정상실행 - 삭제안됨(데이터 없음)
		alert("[변경실패] 해당하는 회원번호가 없습니다.\n");
		location.href = "admin_manage_opr.jsp";
	}
	if (result > 0) { //업데이트 성공
		alert(result + "건 관리자로 되었습니다\n");
		location.href = "admin_manage_opr.jsp";
	}
</script>
</body>
