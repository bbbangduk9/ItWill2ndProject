...
<%//@@@ 게시판 페이징 처리를 하기 위함 @@@ //
	...
	//1. 전체 게시물 수량 구하기
	postPage.setTotalRecord(CommentDAO.getTotalCnt());
...
<%	//현재 페이지 기준으로 DB 데이터(게시글) 가져오기
	...
	List<CommentVO> list = CommentDAO.manageComm(postPage.getBegin(), postPage.getEnd());
...
<div class="secondBar">
	<div class=center>
		...
		<form id="deleteForm" action="admin_del_comment.jsp" method="post">
		       <button type="button" id="del">삭제</button>
		    	<table id="boxTable" border="1">
					<thead>
						<tr>
							<th colspan="2">댓글번호</th>
							<th>댓글 내용</th>
							<th>작성일</th>
						</tr>
					</thead>
			<tbody>
				<c:choose>
					...
					<c:otherwise>
						...
						<tr>
							<td><input type="checkbox" name="commentIdx" class="chk" value="${vo.commentIdx}" ></td>
						<td>${vo.commentIdx }</td>
				...
				<tfoot>
						...
						<td colspan="4">
							...
							<!-- [이전으로]에 대한 사용여부 처리 -->
								...
								<c:if test="${postPage.beginPage != 1 }">
									<li>
										<a href="admin_manage_comment.jsp?cPage=${postPage.beginPage - 1 }">이전으로</a>
									</li>
								</c:if>
							...
							<c:forEach var="pageNo" begin="${postPage.beginPage }" end="${postPage.endPage }">
								...
								<c:if test="${pageNo != postPage.nowPage }">	
								<li>
									<a href="admin_manage_comment.jsp?cPage=${pageNo }">${pageNo }</a>
								</li>
								</c:if>
							</c:forEach>
							...
							<c:if test="${postPage.beginPage < postPage.totalPage }">
								<li>
									<a href="admin_manage_comment.jsp?cPage=${postPage.endPage + 1 }">다음으로</a>
								</li>
							</c:if>
						</ol>
						</td>
